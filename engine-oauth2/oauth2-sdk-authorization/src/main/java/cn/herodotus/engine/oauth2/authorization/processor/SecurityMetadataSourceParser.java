...
public class SecurityMetadataSourceParser {
    ...
    public SecurityMetadataSourceParser(ExpressionParser expressionParser) {
    ...
    private Collection<ConfigAttribute> analysis(SecurityAttribute securityAttribute) {
        ...
        if (StringUtils.isNotBlank(securityAttribute.getManualSetting())) {
            return HerodotusSecurityConfig.createList(securityAttribute.getManualSetting());
        }
        ...
        if (StringUtils.isNotBlank(securityAttribute.getIpAddress())) {
            return HerodotusSecurityConfig.createList(hasIpAddress(securityAttribute.getIpAddress()));
        }
        ...
        if (StringUtils.isNotBlank(securityAttribute.getExpression())) {
            ...
            if (StringUtils.endsWithIgnoreCase(staticExpression, "Role")) {
                ...
                if (StringUtils.isNotBlank(expression)) {
                    return HerodotusSecurityConfig.createList(expression);
                }
            }

            return HerodotusSecurityConfig.createList(staticExpression);
        }

        return HerodotusSecurityConfig.createList(hasAuthority(securityAttribute.getAttributeCode()));
    }
     ...
     * @return 请求和权限的映射的Map
     */
    private LinkedHashMap<HerodotusRequestMatcher, Collection<ConfigAttribute>> convertToSecurityMetadata(String url, String methods, String expression) {
        Collection<ConfigAttribute> configAttributes = HerodotusSecurityConfig.createList(expression);
     ...
     * @return 保存请求和权限的映射的Map
     */
    private LinkedHashMap<HerodotusRequestMatcher, Collection<ConfigAttribute>> convertToSecurityMetadata(String url, String methods, Collection<ConfigAttribute> configAttributes) {
        LinkedHashMap<HerodotusRequestMatcher, Collection<ConfigAttribute>> result = new LinkedHashMap<>();
        if (StringUtils.isBlank(methods)) {
            result.put(new HerodotusRequestMatcher(url), configAttributes);
        } else {
            ...
            if (StringUtils.contains(methods, SymbolConstants.COMMA)) {
                ...
                for (String method : multiMethod) {
                    result.put(new HerodotusRequestMatcher(url, method), configAttributes);
                }
            } else {
                result.put(new HerodotusRequestMatcher(url, methods), configAttributes);
            }
        }
    ...
    private String createExpression(SecurityAttribute securityAttribute, Set<HerodotusGrantedAuthority> relations) {
        if (CollectionUtils.isNotEmpty(relations)) {
            if (StringUtils.containsIgnoreCase(getExpression(securityAttribute), "Any")) {
                String code = relations.stream().map(HerodotusGrantedAuthority::getAuthority).collect(Collectors.joining(SymbolConstants.COMMA));
